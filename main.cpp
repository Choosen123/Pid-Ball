...
int main(){
    ...
    // const char* values[] = {"0", "1", "1", "./trt_cache"};
    // Ort::ThrowOnError(Ort::GetApi().UpdateTensorRTProviderOptions(trt_options, keys, values, 4));
        ...
        if(!cap.read(image_camera)){
            ...
            try{
                    ...
                    if(!output_shape.empty() && !output_data.empty()){
                            ...
                            if(sp.position>=150) sp.position=150;
                            if(sp.position==6) sp.position=0;
